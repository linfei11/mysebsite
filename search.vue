<template>
  <el-card style="width: 100%">
    <el-row>
      ...
      <el-col :span="2">
        <el-button @click="sendSTime" type="success" plain
          >按时间搜索</el-button
        >
      </el-col>
      ...
      <el-col :span="2">
        <el-button @click="sendFuzzy" type="success" plain>模糊搜索</el-button>
      </el-col>
      <el-col :span="2">
...
export default {
  ...
  methods: {
    handleSelectionChange(val) {
      this.selectedDelete = val.map((item) => item.id).join();
    ...
    batchDelete() {
      this.$confirm("此操作将永久删除所选文件, 是否继续?", "提示", {
        ...
        .then(() => {
          this.axios({
            url: "attacks/multiple_delete/",
            method: "delete",
            params: {
              deleteid: JSON.stringify(this.selectedDelete),
            },
          })
            .then((res) => {
              if (res.status == 204) {
