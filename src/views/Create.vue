        ...
        <el-form-item label="pcap文件" prop="pcap_path">
          <el-card>
            <el-upload
              ...
              drag
              class="upload-demo"
              action="http://localhost:8001/attacks/"
              :auto-upload="false"
              :data="create_form"
              :on-success="onSuccess"
              :on-change="handleChange"
              :on-exceed="onErrorUpload"
              name="pcap_path"
              style="padding: 10px"
            >
...
export default {
  ...
  methods: {
    contentChange() {
      ...
      this.create_form.rule_name = myArray[0].split(':')[1].replace("\"", "").replace("\"", "")
    ...
    onErrorUpload() {this.$message.error('upload error')},
    handleChange(file) {
